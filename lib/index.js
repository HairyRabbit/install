...
class App extends Component<*, Props, State> {
   ...
   * @private
   */
  _parser(input: string): Array<any> {
   ...
   */
  componentDidMount() {
    const { libs } = this.props.context

    const subProcess = spawn('yarn.cmd', ['add', 'jquery', '--prefer-offline', '--json', '--ignore-optional', '--ignore-platform', '--ignore-engines'])

    subProcess.stdout.on('data', (data) => {
      const datas = this._parser(data.toString())
  ...
  renderProgressBar(precent, color) {
    return (
      <div>
        [
        <ProgressBar percent={precent} columns={10} color={color} character="=" />
        ]
      </div>
    )
  }
...
render(<App libs={['jquery']} progress={0.5} green />)
